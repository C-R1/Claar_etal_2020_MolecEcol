...
# Create file lists
path <- "data/ITS2/Bioinf/sequences/Claar_etal_2020_ITS2_data" # Path to where sequences are stored
...
ref_seqs <- readDNAStringSet(filepath = "data/ITS2/ITS2db_trimmed_derep_dada.fasta")
min(width(ref_seqs))
max(width(ref_seqs))
...
# Assign taxonomy
sym.ref <- "data/ITS2/ITS2db_trimmed_derep_dada.fasta" 
...
# Import to phyloseq
samdf <- read.delim("data/ITS2/data/mapping_file_dada.txt",header = TRUE) # Read in sample data 
...
# rather than the full DNA sequence when working with some of the 
# tables and visualizations from phyloseq, but we want to keep the 
# full DNA sequences for other purposes like merging with other 
# datasets or indexing into reference databases like the Earth 
# Microbiome Project. For that reason we’ll store the DNA sequences 
# of our ASVs in the refseq slot of the phyloseq object, and then 
# rename our taxa to a short string. That way, the short new taxa 
# names will appear in tables and plots, and we can still recover 
# the DNA sequences corresponding to each ASV as needed with 
# refseq(ps).
...
save(ps, file="analyses/ITS2/dada2/KI_SymMicrobes_dada.RData")
